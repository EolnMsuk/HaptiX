...
### Added
- **Per-trigger haptic style overrides** (`Advanced Settings` section in Settings): Each of the 10 triggers has an independent `PSLinkListCell` letting the user select a specific haptic profile (Light / Medium / Heavy / Soft / Rigid) or fall back to the global Feedback Style ("Use Global"). Ten new integer preference keys (`style_hookKeyboard`, `style_hookButtons`, `style_hookSwitches`, `style_hookCells`, `style_hookScrolling`, `style_hookVolume`, `style_hookPower`, `style_hookLockScreen`, `style_hookIcons`, `style_hookAppSwitcher`) default to `0` (Use Global). Value mapping: `0` = Use Global, `1`–`5` = Light / Medium / Heavy / Soft / Rigid.
- **Info section** (Settings): `PSGroupCell` with GitHub and Donate `PSButtonCell` entries positioned between the Exceptions and Maintenance sections. Implemented via `openGitHub` and `openDonate` action selectors on `HaptixPrefsRootListController` using `UIApplication openURL:options:completionHandler:`.
- **Dynamic version/environment footer** (Settings): A centered secondary-label `UILabel` installed as `tableFooterView` displays the package version and detected jailbreak environment — e.g., `HaptiX v1.1.0 (iOS 16.1.1 RootHide)`. Version sourced from `CFBundleShortVersionString` in the bundle `Info.plist`; environment detected passively by `HXDetectJailbreakEnvironment()` via bundle path prefix (`/var/jb`) and the RootHide marker file (`/var/jb/.installed_roothide`).

### Changed
- **`triggerHaptic()` renamed to `triggerHapticWithOverride(NSInteger overrideStyle)`**: All 13 hook call sites now pass the corresponding `style_hook*` variable. When `overrideStyle == 0` the global `hapticStyle` is used unchanged; values 1–5 map to Light/Medium/Heavy/Soft/Rigid (shifted by 1 to align with zero-indexed `UIImpactFeedbackStyle`).
- **`resetSettings`**: Confirm handler now writes explicit `0` defaults for all 10 `style_hook*` keys in addition to the existing key set, ensuring cfprefsd cache invalidation in long-running processes such as SpringBoard.

### Documentation
- `CHANGELOG.md`, `ProjectStructure.md`, `README.md` updated to reflect new settings sections, function rename, and new preference keys.
